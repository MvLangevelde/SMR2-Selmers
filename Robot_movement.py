    ...
    try:
        ...
        while True:                   
            if switch() <= 3:
                ...
                print('-------------------------')
                robot.movel([0, 0, -0.007, 0, 0, 0], relative = True, vel = 0.003, acc = 1)
                start = robot.getl()
        ...
        robot.movel([-0.080,0.0,0,0,0,0], relative = True, wait = True)#, vel = 2.0, acc = 1.0)
        robot.movel([0, 0, -0.03, 0, 0, 0], relative = True)
        ...
        robot.movel([0,0,y/1000 + 0.080,0,0,0], relative = True)#, vel = 2.0, acc = 1.0)
        robot.movel(startgrinder, vel = 2.0, acc = 1.0)
        
        #Go to a position next to the weld to calibrate the grinder
        robot.movels(quartercirlce(startgrinder), wait = False)
        time.sleep(3)
        robot.stop()
        ...
        robot.movel([0, 0, 0.03, 0, 0, 0], relative = True)#, vel = 2.0, acc = 1.0)
        
        #Move to the x position of the second weldpoint
        robot.movel([0, 0 + x, 0, 0, 0, 0], relative = True)
        
        #Move to the second weldpoint to start grinding
        robot.movel(welposgrinder)
        
        #Make the grinding move
        robot.movels(spiralback(startgrinder, welposgrinder, r), vel = 0.03, acc = 1)
        
        #Continue a bit further
        robot.movel([0.050,0,0,0,0,0], relative = True)
